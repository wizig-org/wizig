...
test "renderZigFfiRoot emits compatibility handshake and structured error symbols" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, rendered, "pub export fn wizig_ffi_wire_format_version() u32") != null);
    ...
    try std.testing.expect(std.mem.indexOf(u8, rendered, "std.fmt.allocPrint(ffi_output_allocator, \"{s}:{s}\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, rendered, "var ffi_pool = BufferPool{};") != null);
...
test "renderKotlinApi and renderAndroidJniBridge emit compatibility and structured errors" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, kotlin_rendered, "private const val WIZIG_EXPECTED_WIRE_FORMAT_VERSION: Int =") != null);
    ...
    try std.testing.expect(std.mem.indexOf(u8, jni_rendered, "extern uint32_t wizig_ffi_wire_format_version(void);") != null);
    ...
    try std.testing.expect(std.mem.indexOf(u8, jni_rendered, "ffi compatibility mismatch: expected abi=%u wire=%u hash=%s got abi=%u wire=%u hash=%s") != null);
