...
pub fn appendBaseSections(
    ...
    // Stack buffer avoids malloc for strings under 512 bytes (covers ~95% of cases).
    try out.appendSlice(arena, "static jstring new_jstring_from_bytes(JNIEnv* env, const uint8_t* bytes, size_t len) {\n");
    try out.appendSlice(arena, "    char stack_buf[512];\n");
    try out.appendSlice(arena, "    char* buf;\n");
    try out.appendSlice(arena, "    if (len < sizeof(stack_buf)) {\n");
    try out.appendSlice(arena, "        buf = stack_buf;\n");
    try out.appendSlice(arena, "    } else {\n");
    try out.appendSlice(arena, "        buf = (char*)malloc(len + 1);\n");
    try out.appendSlice(arena, "        if (buf == NULL) {\n");
    try out.appendSlice(arena, "            throw_structured_error(env, \"wizig.memory\", 2, \"wizig generated bridge out of memory\");\n");
    try out.appendSlice(arena, "            return NULL;\n");
    try out.appendSlice(arena, "        }\n");
    try out.appendSlice(arena, "    }\n");
    try out.appendSlice(arena, "    if (len > 0) memcpy(buf, bytes, len);\n");
    try out.appendSlice(arena, "    buf[len] = '\\0';\n");
    try out.appendSlice(arena, "    jstring result = (*env)->NewStringUTF(env, buf);\n");
    try out.appendSlice(arena, "    if (buf != stack_buf) free(buf);\n");
