...
fn runIos(
    ...
    const executable_name = extractBuildSetting(settings.stdout, "EXECUTABLE_NAME") orelse {
        try stderr.writeAll("error: failed to read EXECUTABLE_NAME from xcodebuild settings\n");
        return error.RunFailed;
    };
    ...
    const simulator_ffi_path = try buildIosSimulatorFfiLibrary(arena, io, stderr);
    ...
    switch (debugger_mode) {
        ...
        .none => {
            const predicate = try std.fmt.allocPrint(arena, "process == '{s}'", .{executable_name});
            try stdout.writeAll("streaming simulator logs (Ctrl+C to stop)...\n");
            try stdout.flush();
            try runInheritChecked(
                io,
                null,
...
fn resolveIosDebugger(
    ...
    return switch (mode) {
        .auto => if (commandExists(arena, io, "lldb"))
            .lldb
        else {
            try stderr.writeAll("error: lldb not found; pass --debugger none or install Xcode command-line tools\n");
            return error.RunFailed;
        },
        .lldb, .none => mode,
        else => {
