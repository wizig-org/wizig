...
const wizig_ios_shell_phase_id = @embedFile("../../../../templates/spec/ios_ffi_phase_id.txt");
...
const wizig_ios_shell_script = @embedFile("../../../../templates/spec/ios_ffi_build.sh");
...
pub fn generateProject(
    ...
    const zig_ffi_root_out = try renderZigFfiRoot(arena, spec);
    ...
    const android_jni_bridge_out = try renderAndroidJniBridge(arena, spec);
...
fn renderZigFfiRoot(arena: std.mem.Allocator, spec: ApiSpec) ![]u8 {
    var out = std.ArrayList(u8).empty;
    errdefer out.deinit(arena);
    ...
    try out.appendSlice(arena, "        error.OutOfMemory => statusCode(.out_of_memory),\n");
    try out.appendSlice(arena, "        else => statusCode(.internal_error),\n");
    try out.appendSlice(arena, "    };\n");
    try out.appendSlice(arena, "}\n\n");
    ...
    for (spec.methods) |method| {
        ...
        if (method.output == .string) {
            switch (method.input) {
                .void => {
                    ...
                    try out.appendSlice(arena, "    if (out_ptr == null or out_len == null) return statusCode(.null_argument);\n");
            ...
            try out.appendSlice(arena, "    const owned = allocator.dupe(u8, value) catch return statusCode(.out_of_memory);\n");
        ...
        if (method.output == .int) {
            ...
            try out.appendSlice(arena, "    if (out_value == null) return statusCode(.null_argument);\n");
...
fn renderSwiftApi(arena: std.mem.Allocator, spec: ApiSpec) ![]u8 {
    ...
    try out.appendSlice(arena, "            throw WizigGeneratedApiError.ffiCallFailed(function: function, status: status)\n");
    try out.appendSlice(arena, "        }\n");
    try out.appendSlice(arena, "    }\n\n");
...
fn renderAndroidJniBridge(arena: std.mem.Allocator, spec: ApiSpec) ![]u8 {
    var out = std.ArrayList(u8).empty;
    errdefer out.deinit(arena);
    ...
    try out.appendSlice(arena, "    if (cls == NULL) return;\n");
    try out.appendSlice(arena, "    char message[192];\n");
    try out.appendSlice(arena, "    snprintf(message, sizeof(message), \"%s failed with status %d\", function_name, (int)status);\n");
    try out.appendSlice(arena, "    (*env)->ThrowNew(env, cls, message);\n");
    try out.appendSlice(arena, "}\n\n");

    try out.appendSlice(arena, "static void throw_oom(JNIEnv* env) {\n");
    try out.appendSlice(arena, "    jclass cls = (*env)->FindClass(env, \"java/lang/OutOfMemoryError\");\n");
    try out.appendSlice(arena, "    if (cls == NULL) return;\n");
    try out.appendSlice(arena, "    (*env)->ThrowNew(env, cls, \"wizig generated bridge out of memory\");\n");
    try out.appendSlice(arena, "}\n\n");
    ...
    try out.appendSlice(arena, "        throw_oom(env);\n");
    try out.appendSlice(arena, "        return NULL;\n");
    try out.appendSlice(arena, "    }\n");
            ...
            .string => {
                ...
                switch (method.input) {
                    ...
                    .string => {
                        ...
                        try appendFmt(&out, arena, "        throw_status_error(env, \"{s}\", 1);\n", .{ffi_name});
                        try out.appendSlice(arena, "        return NULL;\n");
                        try out.appendSlice(arena, "    }\n");
                ...
                try appendFmt(&out, arena, "        throw_status_error(env, \"{s}\", 255);\n", .{ffi_name});
                try out.appendSlice(arena, "        return NULL;\n");
                try out.appendSlice(arena, "    }\n");
