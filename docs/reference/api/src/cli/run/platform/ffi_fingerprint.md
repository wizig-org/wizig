# `src/cli/run/platform/ffi_fingerprint.zig`
...
Bumped to invalidate stale static-archive framework payloads after moving
simulator FFI builds to dynamic Mach-O binaries.

```zig
pub const ios_ffi_cache_version = "wizig-ios-ffi-cache-v4";
```
